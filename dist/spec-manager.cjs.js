...
var SpecHandler = /*#__PURE__*/function () {
  ...
  function SpecHandler(options) {
    ...
    _defineProperty(this, "metaIdSet", new Set());
  ...
  _createClass(SpecHandler, [{
    ...
    value: function addMeta(meta) {
      ...
      self.meta.push(meta);
      self.metaIdSet.add(meta.id); // todo: 考虑是否有必要

      var list = Array.from(meta.list);
    ...
    key: "addSpec",
    value: function addSpec(spec, option) {
      var _this2 = this;

      // 添加quantity
      if (option && option.quantity) {
        this.meta.forEach(function (meta) {
          _this2.addQuantity(meta.key, spec[meta.key], option.quantity);
        });
      } // 添加line


      this.buildLine(spec); // 添加到map中
    ...
    key: "addQuantity",
    value: function addQuantity(specKey, specName, quantity) {
    ...
    value: function buildLine(item) {
      ...
      for (var x = 0; x < l; x++) {
        for (var y = 0; y < l; y++) {
          ...
          var key = item[keys[x]] + '_' + item[keys[y]]; // 根据quantity来创建spec
    ...
    value: function select(spec) {
      ...
      this.meta.forEach(function (meta) {
        _this3.selected[meta.key] = spec[meta.key];
      });
    }
    /**
     * 重置选择的内容
    ...
    value: function getSelectedSpecification() {
      ...
      return this.specMap[keys.map(function (key) {
        return _this5.selected[key];
      }).join('_')];
    }
  }, {
    key: "prepareSpecInfo",
    value: function prepareSpecInfo(key, value) {
      if (!this.map[key]) {
        this.map[key] = {};
      }
